...
export const reviewJourneys: ReviewJourney[] = [
  {
    ...
    expectations: [
      ...
      'Launching a hunt makes the Netlify provisional workflow and GitHub promotion gate feel credible and understandable.',
    ],
    inspiration: [
      'Quiet, editorial density: compact but not cramped.',
      'The UI should make a niche request feel welcome and precise.',
      'The browser experience should make provisional Netlify work distinct from committed public data.',
    ],
    panelQuestions: [
      ...
      'Does the queued Hunt state explain the real architecture without feeling gimmicky?',
    ],
    steps: [
      ...
      { kind: 'click', selector: '#launch-hunt', note: 'Exercise the Netlify queued Hunt path.' },
      { kind: 'hardExpectVisible', selector: 'text=NETLIFY QUEUED HUNT', timeout: 5000 },
      { kind: 'hardExpectVisible', selector: 'text=Promotion still runs through GitHub Actions quality gates', timeout: 5000 },
      { kind: 'snapshot', name: '03-hunt-queued-dom' },
      { kind: 'screenshot', name: '03-hunt-queued' },
    ],
  },
  {
    id: 'technical-design-review-info',
    ...
    expectations: [
      ...
      'The bottom nav should still give a clear route back to the working surfaces.',
    ],
    inspiration: [
      'Documentation can live inside the product when it earns its place.',
      'A technical review surface should make the architecture legible, not verbose.',
      'Info should help a maintainer or curator understand the data flow quickly.',
    ],
    panelQuestions: [
      'Does the Info page read like part of Mosaic or like external documentation?',
      'Is the explanation of approvals and data flow specific enough to be useful?',
      'Can a user get back to Studio quickly from this route?',
    ],
    steps: [
      { kind: 'goto', route: '?/info', note: 'Open the technical design review.' },
      { kind: 'hardExpectVisible', selector: 'text=TECHNICAL DESIGN REVIEW', timeout: 15000 },
      { kind: 'checkVisible', selector: 'text=How Mosaic is wired' },
      { kind: 'checkVisible', selector: 'text=What happens when you press Approve in Studio' },
      { kind: 'snapshot', name: '01-info-dom' },
      { kind: 'screenshot', name: '01-info' },
      { kind: 'click', selector: '#bottom-nav [data-nav="studio"]', when: ['mobile', 'tablet'], note: 'Return to Studio from the bottom nav.' },
      { kind: 'click', selector: '#app-header [data-nav="studio"]', when: ['desktop'], note: 'Return to Studio from the desktop header nav.' },
      { kind: 'hardExpectVisible', selector: 'text=Curation Dashboard', timeout: 15000 },
      { kind: 'snapshot', name: '02-studio-return-dom' },
      { kind: 'screenshot', name: '02-studio-return' },
    ],
  },
  {
    id: 'map-first-load',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        kind: 'checkAnyVisible',
        selectors: ['text=Static batch artifacts', 'text=Research Batches'],
  ...
  },
  {
    ...
    expectations: [
      ...
      'The queued workflow should remain understandable with minimal input.',
    ],
    inspiration: [
    ...
    steps: [
      ...
      { kind: 'hardExpectVisible', selector: 'text=NETLIFY QUEUED HUNT', timeout: 5000 },
      { kind: 'hardExpectVisible', selector: 'text=Promotion still runs through GitHub Actions quality gates', timeout: 5000 },
      { kind: 'snapshot', name: '01-minimal-hunt-queued-dom' },
      { kind: 'screenshot', name: '01-minimal-hunt-queued' },
    ],
  },
  {
